...
$id_array_end = 24; //搜尋到ids中第幾個結束 ，或是下面會改成$ids的陣列長度，若要使用此行，則需把下面的第77行 $id_array_end = count($ids) 注解掉;
...
for ($i = $id_array_start; $i <= $id_array_end - 1; $i++) {
    ...
    for ($j = 1; $j <= $num; $j++) {
        ...
        $data2 = '{"smarc_id": ' . $smarc_id . ', "date":"' . $date . '"}';

        $cmd2 = "get_exedata";

        $public_key = getPublicKey();
        $curl_cmd2 = 'curl -s -k -d "key=%s&cmd=%s&data=%s" -H "Content-Type: application/x-www-form-urlencoded" -X POST %s';
        $curl_cmd2 = sprintf($curl_cmd2, $public_key, $cmd2, addslashes($data2), $url2);
...
$filename = "Exercise-" . date("Y-m-d") . ".csv";
